function avg_gmse = gmse_cv_poly(t, d, X, y, Uy, kernel_type, cv)
% gmse_cv_rbf is the cross-validation function for a polynomial kernel
% 
%   Description
...
fold_gmse = zeros(k_fold, 1);
    ...
    for fold = 1:k_fold
        ...
        K_train_fold = K_matrix(X_train_fold, kernel_type, [t, d]); % kernel matrix over training data
        ...
        K_val_fold = K_matrix(X_train_fold, kernel_type, [t, d],X_val_fold); % kernel matrix over validation data
    ...
    % Average weighted GMSE across all folds
